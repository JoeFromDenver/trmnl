...
* **Responsibilities:** 
  ...
  - **Payload Extraction & Parsing:** Bypassing standard `payload` iterator loops, the engine natively plucks target data arrays surfaced inside the global scope (e.g., `{{ anytime }}`). However, TRMNL's webhook engine type-casts deeply nested JSON structures as primitive `String` sequences rather than native Enumerable arrays. To loop over them securely, the script must extract the variable and convert it explicitly utilizing TRMNL's proprietary `{% assign array = anytime | parse_json %}` filter native tag, breaking the string lock cleanly.
