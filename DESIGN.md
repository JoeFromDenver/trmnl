...
* **Responsibilities:** 
  ...
  - **Payload Extraction:** Bypassing standard `payload` iterator loops, the engine natively plucks target data arrays explicitly surfaced inside the global scope (e.g., `{{ anytime }}`). TRMNL's Polling webhook seamlessly typecasts deep nested JSON schemas directly into native Liquid Enumerable arrays out-of-the-box without manual extraction tags. The `markup.html` pipeline loops over these isolated variables natively, preventing generic generic iterator bloating and securing sequence limits explicitly.
...
4. **CSS Sanitization Vulnerabilities:** TRMNL's rendering framework aggressively strips out or overrides standard Tailwind proxy utility classes (e.g., `text-6xl`, `text-center`, `font-light`), commonly collapsing them into generic `1rem` serif text blocks on the device. To establish precise typography hierarchies, margins, and rendering aesthetics across the strict 800x480 e-ink grid, architects must rigorously inject native HTML `<p style="...">` cascading attributes (e.g., physical `rem` math sizing, `text-align: center`, and structural `font-family: 'Helvetica Neue', Arial, sans-serif` strings) totally bypassing proxy classes natively.
5. **Local Iteration & The Test Harness:** Because local web browsers cannot natively evaluate complex Liquid block logic (causing missing HTML hierarchies and un-rendered tags), attempting to iterate on CSS layouts by opening `markup.html` locally fails completely. To rapidly prototype aesthetics without repeatedly deploying raw code to the active TRMNL device, developers must construct a standalone continuous `test_harness.html`. This document accurately simulates TRMNL's environment by wrapping the raw structural HTML in physical hardware constraints (an absolute `800x480` embedded screen locked inside a simulated darker grey e-ink hardware bezel), natively injecting Tailwind CSS via CDN out-of-the-box, and strictly stripping backend Liquid payloads to safely simulate hardcoded UI mockups.
